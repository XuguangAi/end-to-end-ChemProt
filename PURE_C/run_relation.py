...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = label_id
        self.sub_start_idx = sub_start_idx
        self.sub_end_idx = sub_end_idx
        self.obj_start_idx = obj_start_idx
        self.obj_end_idx = obj_end_idx
...
def convert_examples_to_features(examples, label2id, max_seq_length, tokenizer, special_tokens, unused_tokens=True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(tokens) > max_seq_length:
            ...
            if sub_start_idx >= max_seq_length:
                sub_start_idx = 0
            if sub_end_idx >= max_seq_length:
                sub_end_idx = 0   
            if obj_start_idx >= max_seq_length:
                obj_start_idx = 0
            if obj_end_idx >= max_seq_length:
                obj_end_idx = 0
        else:
            num_fit_examples += 1
        ...
        if num_shown_examples < 20:
            if (ex_index < 5) or (label_id > 0):
                ...
                logger.info("sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx: %d, %d, %d, %d" % (sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx))
...
def evaluate(model, device, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    ...
    nb_eval_steps = 0
    preds = []
    for input_ids, input_mask, segment_ids, label_ids, sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx in eval_dataloader:
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        label_ids = label_ids.to(device)
        sub_start_idx = sub_start_idx.to(device)
        sub_end_idx = sub_end_idx.to(device)
        obj_start_idx = obj_start_idx.to(device)
        obj_end_idx = obj_end_idx.to(device)
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, labels=None, sub_start_idx = sub_start_idx, sub_end_idx = sub_end_idx, obj_start_idx = obj_start_idx, obj_end_idx = obj_end_idx)
...
def main(args):
    ...
    if args.do_eval and (args.do_train or not(args.eval_test)):
        ...
        all_sub_start_idx = torch.tensor([f.sub_start_idx for f in eval_features], dtype=torch.long)
        all_sub_end_idx = torch.tensor([f.sub_end_idx for f in eval_features], dtype=torch.long)
        all_obj_start_idx = torch.tensor([f.obj_start_idx for f in eval_features], dtype=torch.long)
        all_obj_end_idx = torch.tensor([f.obj_end_idx for f in eval_features], dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_sub_start_idx, all_sub_end_idx, all_obj_start_idx, all_obj_end_idx)
    ...
    if args.do_train:
        ...
        all_sub_start_idx = torch.tensor([f.sub_start_idx for f in train_features], dtype=torch.long)
        all_sub_end_idx = torch.tensor([f.sub_end_idx for f in train_features], dtype=torch.long)
        all_obj_start_idx = torch.tensor([f.obj_start_idx for f in train_features], dtype=torch.long)
        all_obj_end_idx = torch.tensor([f.obj_end_idx for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_sub_start_idx, all_sub_end_idx, all_obj_start_idx, all_obj_end_idx)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                input_ids, input_mask, segment_ids, label_ids, sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx = batch
                loss = model(input_ids, segment_ids, input_mask, label_ids, sub_start_idx, sub_end_idx, obj_start_idx, obj_end_idx)
                if n_gpu > 1:
                    loss = loss.mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc', 'chemprot', 'chemprot_5'])
