...
Path = '/Users/XA/Desktop/CHEMPROT/ChemProt_Corpus/chemprot_test_gs' # Path to the training dataset file

nlp = stanza.Pipeline(lang='en', processors='tokenize')

lookup = {"GENE-Y": "GENE", "GENE-N": "GENE", "CHEMICAL": "CHEMICAL"}

df_abstracts = pd.read_table(Path + "/chemprot_test_abstracts_gs.tsv", header=None, keep_default_na=False, names=["doc_key", "title", "abstract"], encoding='utf-8')

df_entities = pd.read_table(Path + "/chemprot_test_entities_gs.tsv", header=None, keep_default_na=False, names=["doc_key", "entity_id", "label", "char_start", "char_end", "text"], encoding='utf-8')

df_relations = pd.read_table(Path + "/chemprot_test_gold_standard.tsv", header=None, keep_default_na=False, names=["doc_key", "label", "arg1", "arg2"], encoding='utf-8')
